...
import 'Toberesults_page.dart';


class TobeQuizPage extends StatefulWidget {
  final String idTem;
  final String Tem;
  const TobeQuizPage({super.key, required this.idTem, required this.Tem, });

  @override
  State<TobeQuizPage> createState() => _QuizPageState(this.idTem, this.Tem);
}

class _QuizPageState extends State<TobeQuizPage> {
  int totalQuestions = 5;
  int totalOptions = 4;
  int questionIndex = 0;
  int progressIndex = 0;
  TobeQuiz quiz = TobeQuiz(name: 'Quiz of Verb Tobe', questions: []);
  ...
  Future<void> readJson() async {
    ...
    while (true) {
      ...
      for (var option in optionList.sublist(1, totalOptions)) {
        otherOptions.add(data[option]['respues']);
      }

      TobeQuestion question = TobeQuestion.fromJson(data[answer]);
        ...
        TextButton(
          onPressed: () {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: ((context) => TobeResultsPage(
                    quiz: quiz,
                    idTem: idTem,
                    Tem: Tem,
                  ))),
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Text(
                        quiz.questions[questionIndex].tobequestion,
